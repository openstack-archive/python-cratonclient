...
class TestProjectsShell(base.ShellTestCase):
    ...
    @mock.patch('cratonclient.v1.projects.ProjectManager.list')
    def test_project_list_sort_key_field_key_success(self, mock_list):
        """Verify --sort-key arguments successfully passed to Client."""
        self.shell('project-list --sort-key name')
        mock_list.assert_called_once_with(
            sort_key='name',
            sort_dir='asc',
        )

    def test_project_list_sort_key_invalid(self):
        """Verify --sort-key with invalid args, fails with Command Error."""
        self.assertRaises(exc.CommandError,
                          self.shell,
                          'project-list --sort-key invalid')

    @mock.patch('cratonclient.v1.projects.ProjectManager.list')
    def test_project_list_sort_dir_asc_success(self, mock_list):
        """Verify --sort-dir asc successfully passed to Client."""
        self.shell('project-list --sort-key name --sort-dir asc')
        mock_list.assert_called_once_with(
            sort_key='name',
            sort_dir='asc'
        )
    ...
    @mock.patch('cratonclient.v1.projects.ProjectManager.list')
    def test_project_list_sort_dir_desc_success(self, mock_list):
        """Verify --sort-dir desc successfully passed to Client."""
        self.shell('project-list --sort-key name --sort-dir desc')
        mock_list.assert_called_once_with(
            sort_key='name',
            sort_dir='desc'
        )

    def test_project_list_sort_dir_invalid_value(self):
        """Verify --sort-dir with invalid args, fails with Command Error."""
        (_, error) = self.shell(
            'project-list --sort-key name --sort-dir invalid'
        )
