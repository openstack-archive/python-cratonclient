...
class TestHostsShell(base.ShellTestCase):
    ...
    host_valid_fields = Namespace(project_id=1, region_id=1, name='mock_host',
                                  ip_address='127.0.0.1', active=True)
    host_invalid_field = Namespace(project_id=1, invalid_foo='ignored')
    ...
    @mock.patch('cratonclient.v1.hosts.HostManager.create',
                return_value=host_valid_fields)
    def test_do_host_create_calls_host_manager_with_fields(self, mock_create):
        ...
        output = hosts_shell.do_host_create(client, self.host_valid_fields)
        self.assertTrue(mock_create.called)
        expected = cliutils.print_dict(self.host_valid_fields.__dict__,
                                       wrap=72)
        self.assertEqual(expected, output)

    @mock.patch('cratonclient.v1.hosts.HostManager.create',
                return_value=host_invalid_field)
    def test_do_host_create_ignores_unknown_fields(self, mock_create):
        """Verify that do host create ignores unknown field."""
        client = mock.Mock()
        client.hosts = hosts.HostManager(mock.ANY, 'http://127.0.0.1/')
        output = hosts_shell.do_host_create(client, self.host_invalid_field)
        self.assertTrue(mock_create.called)
        expected = cliutils.print_dict(self.host_valid_fields.__dict__,
                                       wrap=72)
        self.assertEqual(expected, output)
