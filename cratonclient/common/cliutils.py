...
def variable_updates(resource, args):
    ...
    if not sys.stdin.isatty():
        if update_variables or delete_variables:
            raise exc.CommandError("FIXME cannot set on command line as well!")
        update_variables = json.load(sys.stdin)
    return (update_variables, delete_variables)


def variable_deletes(resource, args):
