...
def get_and_print_variables(resource, args):
    # TODO(jimbaker) add csv, env variable support
    if args.format == 'json':
        print(json.dumps(
            dict(resource.variables.items()),
            sort_keys=True, indent=4))
    else:
        print_dict(resource.variables, wrap=72)
...
def set_variables(resource, args):
    ...
    print("args=%r update=%r delete=%r" % (args.variables, update_variables, delete_variables))
    if update_variables:
