    ...
    def decorator(function):
        ...
        def wrapper(cc, args):
            try:
                ...
                # The pattern is "do_<resource>_(<subject>_)<verb>", like
                # do_project_show or do_project_vars_get.
                parsed = function.__name__.split('_')
                resource = parsed[1]
                verb = parsed[-1]
                subject = parsed[2] if len(parsed) > 3 else None
                msg = 'Failed to {}'.format(verb)
                if subject:
                    # NOTE(thomasem): Subject would be something like "vars" in
                    # "do_project_vars_get".
                    msg = '{} {}'.format(msg, subject)
